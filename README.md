...
A MATLAB implementation for optimizing train speed profiles using dynamic programming algorithms to minimize energy consumption while satisfying operational constraints.
...
├── RecurrenceEquation.m       # Recurrence relation for DP
├── tset.m                     # Test script and visualization
├── MaxCapacityCurve.m         # Maximum capacity speed curve generation
├── GetTractionForce.m         # Train traction force calculation
...
├── RangeDecrease.m            # Speed range constraint calculation
...
- Excel file with track data (speed limits, gradients, curves, stations)

### Quick Start

1. **Clone or download** this repository to your local machine

2. **Prepare track data**: Ensure you have an Excel file named `03-线路数据` with the following sheets:
   - `A1-A14车站` (Station positions)
   - `A1-A14限速` (Speed limits)
   - `A1-A14坡度` (Track gradients)
   - `A1-A14曲线` (Track curves)

3. **Configure parameters**: Edit `Global.m` to set your train parameters:
   ```matlab
   startStation = 6;        % Starting station index
   endStation = 8;          % Destination station index
   M = 194.295;            % Train mass (tons)
   MaxSpeed = 80;          % Maximum speed (km/h)
   T = 220;                % Time constraint (seconds)
   ```

4. **Run optimization**:
   ```matlab
   Global;                          % Load global parameters
   [s,v,F,T,E,Matrix_Jmin] = DynamicProgram();  % Run optimization
   ```

5. **Visualize results**:
   ```matlab
   tset;  % Run test script to plot speed profiles
   ```
...
#### `Global.m`
Defines all global parameters including:
- **Vehicle dynamics**: Mass, efficiency, rotational inertia
- **Force constraints**: Maximum traction/braking forces, acceleration limits
- **Discretization**: Spatial and speed step sizes
- **Operational constraints**: Time limits, boundary conditions
- **Track data**: Preloads speed limits, gradients, and curve information

#### `DynamicProgram.m`
Implements the main dynamic programming algorithm:
- **Input**: Global parameters from `Global.m`
- **Output**: 
  - `s`: Spatial coordinates (m)
  - `v`: Optimized speed profile (km/h)
  - `F`: Train forces at each point (N)
  - `T`: Total running time (s)
  - `E`: Total energy consumption (J)
  - `Matrix_Jmin`: Cost matrix for analysis

#### `RecurrenceEquation.m`
Calculates the cost and feasibility of state transitions:
- **Inputs**: Current speed `v_k`, next speed `v_k_1`, spatial point `k`
- **Outputs**: Cost `J_k`, time `t_k`, force `F_k`, energy `e_k`
- **Features**: Enforces acceleration, force, and speed constraints

### Helper Functions

#### `MaxCapacityCurve.m`
Generates the maximum possible speed profile considering:
- Speed limits along the track
- Train performance characteristics
- Stopping requirements at stations

#### Force and Resistance Calculations
- **`GetTractionForce.m`**: HXD2 locomotive traction characteristics
- **`GetMaxBrakeForce.m`**: Maximum braking force vs. speed
- **`GetBasicResistance.m`**: Basic running resistance (Davis equation)
- **`GetAddResistance.m`**: Additional resistance from gradients and curves

#### Track Data Processing
- **`GetSpeedLimit.m`**: Extracts speed limits for the route
- **`CalculateOneStep.m`**: Forward/backward speed calculation
- **`RangeDecrease.m`**: Computes feasible speed ranges

#### Testing and Visualization
- **`tset.m`**: Test script that:
  - Loads global parameters
  - Runs the optimization
  - Plots speed limit, maximum capacity, and optimized speed profiles
  - Compares different control strategies
...
% Run optimization
[s, v, F, T, E, Matrix_Jmin] = DynamicProgram();

% Display results
fprintf('Optimal running time: %.2f seconds\n', T);
fprintf('Total energy consumption: %.2f MJ\n', E/1e6);
...
xlabel('Distance (m)');
ylabel('Speed (km/h)');
title('Optimized Train Speed Profile');
grid on;

% Plot force profile
figure;
plot(s(1:end-1), F/1000, 'r-', 'LineWidth', 2);
xlabel('Distance (m)');
ylabel('Force (kN)');
...
time_limits = [200, 220, 240, 260];
energy_consumption = zeros(size(time_limits));

for i = 1:length(time_limits)
    Global;  % Reset parameters
    T = time_limits(i);
    lambda_T = 1e8;
    [wj,~,~] = GetAddResistance();
    ...
    [~, ~, ~, ~, E, ~] = DynamicProgram();
    energy_consumption(i) = E;
end

% Plot trade-off curve
figure;
plot(time_limits, energy_consumption/1e6, 'o-', 'LineWidth', 2);
xlabel('Time Limit (s)');
...
### Data Requirements
- **Excel file**: `03-线路数据.xlsx` containing:
  - Station positions (km)
  - Speed limits by track section (km/h)
  - Track gradients (‰)
  - Curve radii (m)

### Hardware Requirements
- **Memory**: Minimum 4GB RAM
- **Processing**: Computation time scales with discretization resolution
- **Storage**: Minimal disk space required

## Notes

1. **Discretization**: The algorithm discretizes both space (10m steps) and speed (0.1 m/s steps). Finer discretization improves accuracy but increases computation time.

2. **Track data format**: Ensure your Excel file follows the expected format with proper column structures for position-based data.

3. **Train parameters**: The current implementation uses HXD2 locomotive characteristics. Modify the traction and braking force functions for different train types.

4. **Optimization objective**: The algorithm minimizes energy consumption while satisfying time constraints. The penalty coefficient `lambda_T` balances energy and punctuality.
