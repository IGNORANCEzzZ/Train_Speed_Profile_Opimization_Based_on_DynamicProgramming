...
% Input parameters:
     ...
     % k: Position index in discrete spatial grid
% Output parameters:
     % curspeed: Speed at x+dx or x-dx, km/h
%%
global M;       % Train mass, t
global g;       % Gravitational acceleration
global step_s;  % Step size, m
global wj;
%%
x_step=step_s;
%%
switch (mode)  
    case 'FP'
        TractionForce=GetTractionForce(preSpeed)*1000; % Original unit KN, multiply by 1000 to get N
        % Get traction force based on speed
        BreakForce=0; 
        % No braking force
    ...
    case 'FB'
        TractionForce=0;
        BreakForce=GetMaxBrakeForce(preSpeed)*1000; % Original unit KN, multiply by 1000 to get N
        % Get maximum braking force based on speed
end
BasicForce=GetBasicResistance(preSpeed)*M*g; % N
% Get train basic resistance based on speed

AdditionalForce=wj(k)*1000; % N
% Get gradient and curve additional resistance for train at this position
...
preSpeed=preSpeed/3.6; % Convert km/h to m/s
    if Forward
        % If Forward=1, curspeed is speed at prePos+dx
        curspeed=sqrt(preSpeed*preSpeed+2*x_step*a)*3.6; % Convert m/s to km/h
